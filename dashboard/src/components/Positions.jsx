...
function Positions() {
  ...
  return (
    <div className="h-full w-full p-6">
      ...
      {loading ? (
        <p className="text-gray-500 py-6">Loading positions data...</p>
      ) : (
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 text-gray-600 text-sm">
                ...
                <th className="py-3 px-4 text-center font-semibold">Instrument</th>
                <th className="py-3 px-4 text-center font-semibold">Qty.</th>
                <th className="py-3 px-4 text-center font-semibold">Avg.</th>
                <th className="py-3 px-4 text-center font-semibold">LTP</th>
                <th className="py-3 px-4 text-center font-semibold">P&amp;L</th>
                <th className="py-3 px-4 text-center font-semibold">Chg.</th>
              </tr>
            </thead>
            <tbody>
              {allPositions.map((stocks, index) => {
                ...
                return (
                  <tr className="border-b border-gray-100 hover:bg-gray-50" key={stocks._id || index}>
                    <td className="py-3 px-4 text-gray-600">{stocks.product}</td>
                    <td className="py-3 px-4 text-center font-medium text-gray-800">{stocks.name}</td>
      ...
      {allPositions.length > 0 && <Vertical className="mt-8" data={data} />}
    </div>
  );
}
